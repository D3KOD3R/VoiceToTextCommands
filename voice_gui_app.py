...
from tkinter import BOTH, DISABLED, END, LEFT, NORMAL, RIGHT, Canvas, Listbox, StringVar, Tk, messagebox, ttk
    ...
    def start(self, device: int | None, samplerate: int | None = None, channels: int | None = None) -> None:
        ...
        def callback(indata, frames, time_info, status):  # type: ignore[no-untyped-def]
            ...
            with self._lock:
                ...
                self.level_history = self.level_history[-80:]
...
class VoiceGUI:
    def __init__(self) -> None:
        ...
        self.record_level_history: list[float] = []
    ...
    def _build_layout(self) -> None:
        ...
        self.issue_listbox = Listbox(pending_frame, height=16, width=40, selectmode="extended")
        ...
        self.issue_listbox_done = Listbox(done_frame, height=16, width=40, selectmode="extended")
        ...
        row.pack(fill=BOTH, padx=10, pady=(2, 2))
        ttk.Label(row, text="Input device:").pack(side=LEFT, padx=(0, 6))
        if self.device_list:
            self.device_combo.current(0)
            self.device_combo.bind("<<ComboboxSelected>>", self.on_device_change)
        self.device_combo.pack(side=LEFT, padx=(8, 0))
        ttk.Button(row, text="Refresh", command=self.refresh_devices).pack(side=RIGHT)

        self.device_label = ttk.Label(self.controls_frame, text=f"Selected: {self.selected_device_name}", justify=LEFT)
        self.device_label.pack(anchor="w", padx=10, pady=(0, 2))
        ...
        ttk.Entry(path_row, textvariable=self.repo_path_var, width=46).pack(side=LEFT, padx=(0, 10))
        ...
        ttk.Entry(issue_path_row, textvariable=self.issues_path_var, width=46).pack(side=LEFT, padx=(0, 10))
        ...
        self.live_indicator.pack(in_=self.controls_frame, anchor="w", padx=10, pady=(0, 5))
        ...
        self.test_canvas.config(height=140)
        self.test_canvas.pack(in_=self.controls_frame, fill=BOTH, expand=False, padx=10, pady=(0, 5))
    ...
    def _poll_level(self) -> None:
        if self.mic_tester.is_testing():
            ...
            self._draw_test_history(self.mic_tester.level_history)
            ...
            self._draw_level_bar(level)
            self.record_level_history.append(level)
            self.record_level_history = self.record_level_history[-200:]
            self._draw_test_history(self.record_level_history)
    ...
    def _draw_test_history(self, history: list[float]) -> None:
        ...
        for i, level in enumerate(history[-(width // bar_width) :]):
            ...
            y1 = height
            color = "#4caf50" if level > self.mic_tester.threshold else "#888888"
